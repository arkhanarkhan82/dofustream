...
# Directories (UPDATED FOR NEW CMS STRUCTURE)
TSDB_DIR = os.path.join(CMS_ROOT, "assets", "logos", "tsdb")
STREAMED_DIR = os.path.join(CMS_ROOT, "assets", "logos", "streamed")
LEAGUE_DIR = os.path.join(CMS_ROOT, "assets", "logos", "leagues")

# REFRESH SETTINGS
REFRESH_DAYS = 60

# Context for SSL (ignore errors)
    ...
    try:
        ...
        with urllib.request.urlopen(req, context=SSL_CONTEXT, timeout=10) as response:
            if response.status == 200:
                ...
                # Check if it's an image (simple magic bytes check optional, or just trust content-type)
                # We won't resize since we don't have PIL, but we just save bytes.
...
def download_multi_source(source_obj, save_path):
    # Only skip if we have the file and it's fresh
    ...
    for raw_url in urls:
        ...
        if download_file(final_url, save_path):
            return True
...
def main():
    ...
    print(f"--- Starting Backend Asset Sync (All Teams) ---")
    print(f"🌍 API: {api_url}")
    
    try:
        # Fetch Match Data using urllib
        req = urllib.request.Request(api_url, headers=HEADERS)
        with urllib.request.urlopen(req, context=SSL_CONTEXT, timeout=10) as response:
            if response.status != 200:
                 print(f"CRITICAL: API Error {response.status}")
                 return
    ...
    print(f"🔍 Processing {len(matches)} matches for images...")
    ...
    for m in matches:
        ...
        for raw_name, img_obj in [(home_raw, home_imgs), (away_raw, away_imgs)]:
            ...
            if not os.path.exists(tsdb_path):
                ...
                # Try to download if we have images
                if img_obj:
                    if download_multi_source(img_obj, streamed_path):
                        team_count += 1
    ...
    print(f"--- Sync Done. Teams Downloaded: {team_count} | Leagues Downloaded: {league_count} ---")
